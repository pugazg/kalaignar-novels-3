---
scan_page: 25
part: 1
part_page: 25
printed_page: 8
work: "ponnar-sankar"
section: "மண விழாவில் மச்சான்"
page_type: "chapter-closing"
status: "needs-review"
visual_fidelity: "needs-review"
language: "ta"
source_filename: "TVA_BOK_0065560_பொன்னர்_சங்கர்_2017_part_001_pages_1-75.pdf"
transcription_method: "direct source-pixel transcription; Part001 Pass 1 batch 3"
---

# மண விழாவில் மச்சான் — முடிவு

## Source transcription

என்று அலறியபடி! வதங்கிப்போன அல்லித்தண்டுபோல தரையில் கிடந்த அந்த வாலிபனைக் கொண்டுபோய் கரையானும் செல்லும் கடிக்கக்கூடிய ஒரு கொட்டகையில் போட்டுப் பூட்டுமாறு உத்தரவிட்டார் மலைக்கொழுந்தாக் கவுண்டர். மயக்கத்திலிருந்த அவனை நாலுபேர் தூக்கிக் கொண்டுபோய் அந்தப் பயங்கரக் கொட்டடியில் அடைத்தனர்.

“செல்லாத்தாக் கவுண்டருக்கு இந்தச் சேதியெல்லாம் தெரிய வேண்டாம். தாமரையை உடனே தூக்கிக் கொண்டு போய் சிகிச்சை செய்” என்று மனைவியிடம் கூறிவிட்டு மலைக்கொழுந்தாக் கவுண்டர் வேகமாக வெளியேறினார்.

## Pass 1 notes

- source-visible printed page number: **8** at upper left;
- running header at upper right: கலைஞர் மு. கருணாநிதி;
- opening directly continues scan24;
- substantial blank lower field contains a blue two-warrior illustration;
- this closes chapter1 மண விழாவில் மச்சான்;
- status and visual fidelity remain needs-review.

<!-- மூல மொத்த ஸ்கேன் பக்கம்: 25; பகுதி: 001; பகுதி உள்ளூர் பக்கம்: 25; அச்சுப் பக்கம்: 8; PASS 1 TEXT-COMPLETE / needs-review -->

## Formal Part001 Pass 2A review

- direct word-by-word, punctuation, paragraph/dialogue, printed-pagination and physical page-boundary comparison completed against rendered source pixels;
- corrected `அந்தப் புழங்காக் கொட்டடியில்` → source-visible `அந்தப் பயங்கரக் கொட்டடியில்`;
- source-text corrections: **1**;
- unresolved textual questions: **0**;
- Pass 2A result: **REVIEWED / PASS**;
- page remains `status: "needs-review"` / `visual_fidelity: "needs-review"` pending later gates.

## Formal Part001 Pass 2B review

- independent lexical / historical-glyph reread completed directly against rendered source pixels;
- word boundaries, spacing, punctuation-sensitive readings, historical/source-visible glyph identity and compounds were rechecked without modernization;
- source-text corrections in Pass 2B: **0**;
- unresolved textual questions: **0**;
- Pass 2B result: **REVIEWED / PASS**;
- page remains `status: "needs-review"` / `visual_fidelity: "needs-review"` pending later gates.

## Formal Part001 Pass 3 review

- meaningful full-page visual / structural verification completed directly against rendered source pixels;
- printed page number `8` appears at upper left and running author header `கலைஞர் மு. கருணாநிதி` at upper right above the horizontal rule;
- closing prose occupies the upper portion while a substantial blank lower field contains a blue two-warrior illustration;
- chapter-closing classification, illustration placement, blank-field treatment and printed pagination are structurally correct;
- structural corrections in Pass 3: **0**;
- unresolved visual / structural questions: **0**;
- Pass 3 result: **REVIEWED / PASS**;
- page remains `status: "needs-review"` / `visual_fidelity: "needs-review"` pending whole-Part audit and final metadata promotion.
